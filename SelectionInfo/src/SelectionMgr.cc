...
void
SelectionMgr::SetIndex( const int& i )
{
    _idx = i;
}

void
SelectionMgr::SetRoot( TChain* ch )
{
    vtx.Register( ch );
    lep.Register( ch );
    evt.Register( ch );
...
*******************************************************************************/
bool
SelectionMgr::passHLT( vector<int> hlt )
{
    for( auto h : hlt ){
...
TLorentzVector
SelectionMgr::getLorentzVector( const string& type, const int& idx )
{
    TLorentzVector tl;
...
TLorentzVector
SelectionMgr::getLorentzLep( const int& idx )
{
    return getLorentzVector( "lep", idx );
}

vector<TLorentzVector>
SelectionMgr::getLorentzJet( const vector<int>& idx )
{
...
bool
SelectionMgr::isIsoLepton( const int& lepidx, const int& jetidx )
{
...
TLorentzVector
SelectionMgr::getMET( const TLorentzVector lep )
{
...
double
SelectionMgr::Phi_mpi_pi( double x )
{
...
double
SelectionMgr::getLepPt( const int& idx )
{
    return lep.Pt[ idx ];
}

double
SelectionMgr::getJetPt( const int& idx )
{
    return jet.Pt[ idx ];
}
...
*******************************************************************************/
int
SelectionMgr::matchBhandle( const int& idx, const int& charge )
{
...
int
SelectionMgr::getMuonCharge( const int& idx )
{
    return lep.Charge[ idx ];
}

int
SelectionMgr::bbarDeltaR( const int& bidx, const int& charge )
{
    int idx   = -1;
    double dR = 999;
...
void
SelectionMgr::RvalueUP( const double& up )
{
    _rvalue += up;
}

void
SelectionMgr::getRvalue()
{
    cout << "Rvalue " << _rvalue << endl;
}

int
SelectionMgr::matchGenlevel( const float& eta, const float& phi )
{
    double deta   = 0;
    double dphi   = 0;
    double deltaR = 0;
...
void
SelectionMgr::showGenInfo( const int& i )
{
...
void
SelectionMgr::showJetInfo( const int& i )
{
...
int
SelectionMgr::getDirectDa1( int idx )
{
    if( gen.Da1[ idx ] < 0 ){
        return -999;
    }
...
int
SelectionMgr::getDirectDa2( int idx )
{
    if( gen.Da2[ idx ] < 0 ){
        return -999;
    }
...
int
SelectionMgr::getDirectMother( int idx )
{
    if( gen.Mo1[ idx ] < 0 ){
        return -999;
    }
...
int
SelectionMgr::getDirectMotherPdgID( const int& idx )
{
    return gen.PdgID[ getDirectMother( idx ) ];
}

bool
SelectionMgr::isXGenParticle( const int& idx, const int& pdgid )
{
    return fabs( gen.PdgID[ idx ] ) == pdgid;
}

int
SelectionMgr::getPdgID( const int& idx )
{
...
bool
SelectionMgr::isCommonMo( const int& idx1, const int& idx2, const int& pdgid )
{
...
void
SelectionMgr::cleanHandle()
{
    bhandle.clear();
}

bool
SelectionMgr::checkPartonTopo()
{
    for( int i = 0; i < gsize(); i++ ){
